...
- **找最近**：点「定位找空位」并允许获取位置后，按直线距离排序，最近的有空位站点会单独标出。位置默认只在浏览器里参与计算，不上传、不保存。
- **核对登记位置**：定位后展开站点，会显示「你当前位置距本站约 xx 米」；若你正好站在某个站点旁边，页面会直接提示。实际桩位不在登记位置时，点「登记位置有误？上报」——这一步会把你本次定位、定位精度、站点在各坐标系下的登记值写进一个**公开的** GitHub issue（只有你主动点才会发出，页面自己不发送、不存储）。
- **搜索**：输入站点名、别名或设备号，如「教七」「微电子学院」「60659111」，命中关键词会高亮。别名是校园地图与楼宇的叫法（如「微电子学院」= 行政楼北侧、「内单」= 新桥门南侧），对应关系见 [待确认事项](./docs/open-items.md)。
- **筛选**：全部 / 有空位 / 使用中 / 有故障。
...
**实测（2026-09-18）**：玉泉 14 个站点、35 台设备都能返回状态；每台 12 个插座共 420 个孔位，状态串长度与孔位数一致；35 秒内有 2 台设备状态发生变化 —— 数据是实时的。上游偶发单台超时或 502（实测一轮 35 台中会出现 0–2 台），此时该设备单独计 0 孔、面板会标注，下次轮询自动恢复；因此页面上「孔位合计」偶尔小于 420。
